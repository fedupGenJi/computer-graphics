...
- Translucent cube = original (unrotated) cube at origin
- Solid cube       = rotated cube using hard-coded (RX, RY, RZ)
- Rotate the whole coordinate system with mouse drag to inspect in 3D
...
def display():
    ...
    # Scene rotation (mouse-controlled)
    ...
    # Solid rotated cube (apply rotation around EACH axis)
    glPushMatrix()
    # Rotate around X then Y then Z (explicit per-axis rotation)
...
def main():
    ...
    glutCreateWindow(b"3D Rotation: Original (Translucent) vs Rotated (Solid)")

    init_gl()

    print("3-D rotation (RX, RY, RZ):", (RX, RY, RZ))
